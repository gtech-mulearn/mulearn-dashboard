  ...
  return (
    ...
    >
      ...
      <div className="absolute left-5 right-5 top-5 flex items-start justify-between z-10 transition-transform duration-500 group-hover:-translate-y-1">
        <div className="rounded-full bg-muted/80 px-4 py-1.5 shadow-md">
          <p className="text-[10px] font-extrabold uppercase tracking-widest">
            {group.category}
          </p>
        </div>
        {group.code && (
          <div className="rounded-full bg-muted/80 px-3 py-1.5 shadow-md">
            <p className="text-xs font-bold tracking-widest">{group.code}</p>
          </div>
        )}
      </div>

      <div className="absolute bottom-4 left-4 right-4 p-5 transition-all duration-500 group-hover:-translate-y-2">
        <div className="flex flex-col gap-3 text-left">
          <h3 className="text-2xl font-black text-primary-foreground line-clamp-2">
            {group.name}
          </h3>
        </div>
      </div>
