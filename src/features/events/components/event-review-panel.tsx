...
export function EventReviewPanel({ event }: EventReviewPanelProps) {
  ...
  const getOrganizerName = () => {
    ...
    if (type === "global_ig") {
      return event.organizer.ig?.name ?? event.organizer.organiser_ig?.name ?? "Global IG";
    }
    if (type === "campus_ig") {
      const igName = event.organizer.ig?.name ?? event.organizer.organiser_ig?.name;
      const campusInfo = event.organizer.campus ?? event.organizer.organiser_campus;
    ...
    if (type === "company") {
      const companyInfo = event.organizer.company ?? event.organizer.organiser_company;
  ...
  const reviewRows = [
    ...
    { label: "Scope", value: event.scope_ig?.name || event.scope_org?.title || event.scope || "Not set" },
    ...
    { label: "Venue", value: event.venue.type === "online" ? "Online" : (event.venue.address || event.venue.city || "Venue TBA") },
    { label: "Online Link", value: event.venue.online_link || "Not set" },
    { label: "Platform", value: event.venue.platform || "Not set" },
    { label: "Tags", value: event.tags ? JSON.stringify(event.tags) : "Not set" },
    ...
    { label: "Registration Deadline", value: event.registration_deadline ? formatDate(event.registration_deadline) : "Not set" },
  ];
    ...
    <Card className="rounded-2xl border-border bg-card lc-card-shadow">
      ...
      <CardContent>
        <div className="space-y-0">
          {reviewRows.map((row) => (
            <div key={row.label} className="border-b border-border px-0 py-3 last:border-b-0">
